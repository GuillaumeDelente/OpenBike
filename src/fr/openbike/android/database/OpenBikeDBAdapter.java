...
public class OpenBikeDBAdapter {
	...
	private static final String CREATE_NETWORKS_TABLE = "CREATE TABLE "
			...
			+ " text not null, " + KEY_VERSION + " integer not null);";
	...
	public void cleanAndInsertStations(long version, JSONArray jsonBikes)
		...
		mDb.delete(STATIONS_TABLE, KEY_NETWORK + " = ?", new String[] { String
				.valueOf(network) });
		mDb.delete(STATIONS_VIRTUAL_TABLE, KEY_NETWORK + " = ?",
				new String[] { String.valueOf(network) });
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			if (oldVersion == 2) {
				try {
					db.beginTransaction();
					// Create temporary table
					db.execSQL("CREATE TABLE " + NETWORKS_TABLE + " ("
							+ BaseColumns._ID + " integer primary key, "
							+ KEY_NAME + " text not null, " + KEY_CITY
							+ " text not null, " + KEY_LATITUDE
							+ " integer not null, " + KEY_LONGITUDE
							+ " integer not null, " + KEY_SERVER
							+ " text not null, " + KEY_SPECIAL_NAME
							+ " text not null);");
					...
					db.execSQL("CREATE VIRTUAL TABLE " + STATIONS_VIRTUAL_TABLE
			...
			if (oldVersion == 3) {
				try {
					db.beginTransaction();
					db.execSQL("ALTER TABLE networks ADD COLUMN " + KEY_VERSION
